...
import org.pucar.service.UserService;
...
public class AdvocateRegistrationEnrichment {
    ...
    public void enrichAdvocateRegistration(AdvocateRequest advocateRequest) {
        try {
            List<String> advocateRegistrationIdList = idgenUtil.getIdList(advocateRequest.getRequestInfo(), advocateRequest.getCases().get(0).getTenantId(), "product.id", "P-[cy:yyyy-MM-dd]-[SEQ_PRODUCT_P]", advocateRequest.getCases().size());
            Integer index = 0;
            for(Advocate advocate : advocateRequest.getCases()){
