...
public class ClerkApiController {
	...
	public ResponseEntity<AdvocateClerkResponse> clerkV1SearchPost(
		...
		if (accept != null && accept.contains("application/json")) {
			try {
				List<AdvocateClerk> applications = advocateClerkService.searchAdvocateApplications(body.getRequestInfo(), body.getCriteria(), body.getStatus(), body.getApplicationNumber());
				ResponseInfo responseInfo = responseInfoFactory.createResponseInfoFromRequestInfo(body.getRequestInfo(), true);
