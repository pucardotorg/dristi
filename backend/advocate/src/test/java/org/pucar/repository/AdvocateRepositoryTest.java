...
public class AdvocateRepositoryTest {
    ...
    public void testGetApplications() {
        ...
        when(queryBuilder.getAdvocateSearchQuery(anyList(), anyList(),anyList(), anyString(), any(),any(),any(),any())).thenReturn("SELECT * FROM advocates WHERE condition = ?");
        ...
        List<Advocate> result = repository.getApplications(searchCriteria, statusList, applicationNumber, new AtomicReference<>(),1,0, new Pagination());

        // Verify that queryBuilder was called with correct arguments
        verify(queryBuilder).getAdvocateSearchQuery(eq(searchCriteria), anyList(), anyList(), anyString(), any(),any(),any(),any());
