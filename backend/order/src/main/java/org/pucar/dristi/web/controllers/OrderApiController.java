...
public class OrderApiController {
    ...
    public ResponseEntity<OrderListResponse> orderV1SearchPost(@Parameter(in = ParameterIn.QUERY, description = "the applicationNumber whose order(s) are being queried", schema = @Schema()) @Valid @RequestParam(value = "applicationNumber", required = false) String applicationNumber,
                                                               ...
                                                               @Parameter(in = ParameterIn.QUERY, description = "the orderType of the order(s) being searched", schema = @Schema()) @Valid @RequestParam(value = "orderType", required = false) String orderType,
        ...
        try {
            List<Order> orders = orderService.searchOrder(orderNumber,applicationNumber, cnrNumber, filingNumber, tenantId, id, status, orderType, requestInfo);
