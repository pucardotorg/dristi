...
public class TaskQueryBuilder {
    ...
    public String getTaskSearchQuery(TaskCriteria criteria, List<Object> preparedStmtList, List<Integer> preparedStmtArgList) {
        try {
            ...
            firstCriteria = addListCriteria(status, query, firstCriteria, preparedStmtList,preparedStmtArgList);
    ...
    private static void prepareStatementAndArgumentForListCriteria(List<String> itemList, StringBuilder query, List<Object> preparedStmtList, List<Integer> preparedStmArgList) {
        if (!itemList.isEmpty()) {
            query.append("task.status").append(" IN (")
                    .append(itemList.stream().map(id -> "?").collect(Collectors.joining(",")))
                    .append(")");
            itemList.forEach(i->preparedStmArgList.add(Types.VARCHAR));
