...
public class TaskApiController {
    ...
    public ResponseEntity<TaskListResponse> taskV1SearchPost(@NotNull @Parameter(in = ParameterIn.DEFAULT, description = "RequestInfo for the creation of task", schema = @Schema()) @Valid @RequestBody RequestInfo requestInfo,
                                                             @NotNull @Parameter(in = ParameterIn.QUERY, description = "the order ID whose task(s) are being queried", required = true, schema = @Schema()) @Valid @RequestParam(value = "orderId", required = true) UUID orderId,
                                                             @NotNull @Parameter(in = ParameterIn.QUERY, description = "the cnrNumber of the case whose task(s) are being queried", required = true, schema = @Schema()) @Valid @RequestParam(value = "cnrNumber", required = true) String cnrNumber,
        ...
        List<Task> tasks = taskService.searchTask(id, tenantId, status, orderId, cnrNumber, taskNumber,requestInfo);
