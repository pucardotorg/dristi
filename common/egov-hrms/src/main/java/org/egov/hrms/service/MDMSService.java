...
public class MDMSService {
		...
		if(null != response) {
			if(!CollectionUtils.isEmpty(response.getMdmsRes().keySet())) {
				if(null != response.getMdmsRes().get(HRMSConstants.HRMS_MDMS_COMMON_MASTERS_CODE)){
					...
					masterData.put(HRMSConstants.HRMS_MDMS_DISTRICT_CODE, eachMasterMap.get(HRMSConstants.HRMS_MDMS_DISTRICT_CODE));
				}
	...
	public MdmsCriteriaReq prepareMDMSRequest(StringBuilder uri, RequestInfo requestInfo, String tenantId) {
		...
		String[] commonMasters = {HRMSConstants.HRMS_MDMS_DEPT_CODE, HRMSConstants.HRMS_MDMS_DESG_CODE, HRMSConstants.HRMS_MDMS_YEAR_CODE,HRMSConstants.HRMS_MDMS_COURT_CODE,HRMSConstants.HRMS_MDMS_COURT_ESTABLISHMENT, HRMSConstants.HRMS_MDMS_DISTRICT_CODE};
